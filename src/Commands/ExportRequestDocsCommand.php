...
namespace Piovezanfernando\LaravelApiAutoDocs\Commands;
...
class ExportRequestDocsCommand extends Command
{
    ...
    private LaravelApiAutoDocs $laravelRequestDocs;
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $excludedMethods = config('api-auto-docs.open_api.exclude_http_methods', []);
    ...
    private function confirmFilePathAvailability(): bool
    {
        ...
        if (!$path) {
            $path = config('api-auto-docs.export_path', 'api.json');
        }
        ...
        if (file_exists($this->exportFilePath)) {
            if (!$this->option('force')) {
                return $this->confirm("File exists on [$path]. Overwrite?") === true;
            }
        }
