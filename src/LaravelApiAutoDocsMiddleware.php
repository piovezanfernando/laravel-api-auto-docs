...
class LaravelApiAutoDocsMiddleware extends QueryLogger
{
    /**
    ...
    {
        if (! config('api-auto-docs.enabled')) {
            return $next($request);
        }

        if (! config('app.debug') && $request->headers->has('X-Auto-Doc')) {
        ...
        if (! $request->headers->has('X-Auto-Doc')) {
            return $next($request);
        }

        if (! config('api-auto-docs.hide_sql_data')) {
            $this->listenToDB();
        }

        if (! config('api-auto-docs.hide_logs_data')) {
            $this->listenToLogs();
        }

        if (! config('api-auto-docs.hide_models_data')) {
            $this->listenToModels();
        }
