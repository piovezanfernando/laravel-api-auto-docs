...
class LaravelApiAutoDocsToOpenApi
{
    private array $openApi = [];

    /**
     * @param \Piovezanfernando\LaravelApiAutoDocs\Doc[] $docs
     * @return $this
     */
    public function openApi(array $docs): LaravelApiAutoDocsToOpenApi
    {
        $this->openApi['openapi']                 = config('api-auto-docs.open_api.version', '3.0.0');
        $this->openApi['info']['version']         = config('api-auto-docs.open_api.document_version', '1.0.0');
        $this->openApi['info']['title']           = config('api-auto-docs.open_api.title', 'Laravel Api Auto Doc');
        $this->openApi['info']['description']     = config('api-auto-docs.open_api.description', 'Laravel Api Auto Doc');
        $this->openApi['info']['license']['name'] = config('api-auto-docs.open_api.license', 'Apache 2.0');
        $this->openApi['info']['license']['url']  = config('api-auto-docs.open_api.license_url', 'https://www.apache.org/licenses/LICENSE-2.0.html');
        $this->openApi['servers'][]               = [
            'url' => config('api-auto-docs.open_api.server_url', config('app.url'))
        ];
    ...
    {
        $this->openApi['paths'] = [];
        $deleteWithBody         = config('api-auto-docs.open_api.delete_with_body', false);
        $excludeHttpMethods     = array_map(fn($item) => strtolower($item), config('api-auto-docs.open_api.exclude_http_methods', []));
    ...
    protected function setAndFilterResponses(Doc $doc): array
    {
        ...
        $configResponses = config('api-auto-docs.open_api.responses', []);
    ...
    {
        $securityType = config('api-auto-docs.open_api.security.type');
        ...
        switch ($securityType) {
            case 'bearer':
                $this->openApi['components']['securitySchemes']['bearerAuth'] = [
                    'type' => 'http',
                    'name' => config('api-auto-docs.open_api.security.name', 'Bearer Token'),
            ...
            case 'basic':
                $this->openApi['components']['securitySchemes']['basicAuth'] = [
                    'type' => 'http',
                    'name' => config('api-auto-docs.open_api.security.name', 'Basic Username and Password'),
            ...
            case 'apikey':
                $this->openApi['components']['securitySchemes']['apiKeyAuth'] = [
                    'type' => 'apiKey',
                    'name' => config('api-auto-docs.open_api.security.name', 'api_key'),
                    'in' => config('api-auto-docs.open_api.security.position', 'header'),
            ...
            case 'jwt':
                $this->openApi['components']['securitySchemes']['bearerAuth'] = [
                    'type' => 'http',
                    'scheme' => 'bearer',
                    'name' => config('api-auto-docs.open_api.security.name', 'Bearer JWT Token'),
                    'in' => config('api-auto-docs.open_api.security.position', 'header'),
